...
int main()
{
	...
	printGroup(group, n);
	std::cout << "Сортировка по фамилии" << std::endl;
	Sort(group, n, CompareStudentsByl_name);
	printGroup(group, n);
	std::cout << "Сортировка по среднему баллу" << std::endl;
	Sort(group, n, CompareStudentsBya_score);
	printGroup(group, n);
	
	std::cout << "Количество имен больше \"с\" " << std::endl;
	std::cout<< CountIf (group, n, StudentCompareName) << std::endl;
	std::cout << "Количество фамилий больше \"с\" " << std::endl;
	std::cout << CountIf(group, n, StudentCompareLname) << std::endl;
	std::cout << "Количество учеников со средним больше 8 " << std::endl;
...
struct Student* inputData(int n)
{
	...
	std::cout << "Имя студента номер " << n << " ";
	std::cin >> group->name;
	std::cout << "Фамилия студента номер " << n << " ";
	std::cin >> group->l_name;
	std::cout << "Средний балл студента номер " << n << " ";
...
void StudentsTwoCompareFunc(Student**group)
{
	...
	std::cout << "Критерий поиска 1-Имя 2-Фамилия 3-Средний балл 4-выход" << std::endl;
	while (criterion > 4 || criterion < 1) {
		std::cin >> criterion;
	}
	if (criterion == 4) return;
	std::cout << " N студента 1 и N студента 2 ?" << std::endl;
	std::cin >> x1 >> x2;
